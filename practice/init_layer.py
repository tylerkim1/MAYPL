...
class SimpleInitLayer(nn.Module):
    ...
    def _init_parameters(self):
        """Xavier 초기화"""
        nn.init.xavier_uniform_(self.proj_ent_to_rel.weight, gain=nn.init.calculate_gain('relu'))
        nn.init.xavier_uniform_(self.proj_rel_to_ent.weight, gain=nn.init.calculate_gain('relu'))
        nn.init.zeros_(self.proj_ent_to_rel.bias)
        nn.init.zeros_(self.proj_rel_to_ent.bias)
    ...
    def forward(self, emb_ent: torch.Tensor, emb_rel: torch.Tensor,
        ...
        # - proj_ent_to_rel(emb_ent)를 계산 (활성화 함수 없음 - 원본 MAYPL 방식)
        ...
        # - heads와 tails에 해당하는 메시지를 연결 후 평균
        msg_ent_to_rel = (heads_to_rel + tails_to_rel) / 2
        ...
        msg_count_rels = torch.clamp(msg_count_rels, min=1)  # 0으로 나누기 방지
        ...
        # - proj_rel_to_ent(new_emb_rel)를 계산 (활성화 함수 없음 - 원본 MAYPL 방식)
        rels_emb = torch.index_select(new_emb_rel, 0, rels)
        msg_rel_to_ent = self.proj_rel_to_ent(rels_emb)
        ...
        # msg_count_heads = torch.clamp(msg_count_heads, min=1)  # 0으로 나누기 방지
        msg_count_tails = torch.bincount(tails, minlength=self.num_ent).unsqueeze(1).to(emb_ent.device).float()
        # msg_count_tails = torch.clamp(msg_count_tails, min=1)  # 0으로 나누기 방지
        msg_count_ents = msg_count_heads + msg_count_tails
        msg_count_ents = torch.clamp(msg_count_ents, min=1)
